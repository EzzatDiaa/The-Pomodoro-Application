...
def start_time():
    ...
    if reps % 8 == 0:
        ...
        count_down(work_sec)
        tittle_label.config(text="Working", fg=GREEN)
...
def count_down(count):
    ...
    # I was abel to do this because of Dynamic Typing of python

    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)
    else:
        start_time()  # here we need to rest the work time and start the break time,
        # So the time needed to be caught when it goes down to zero and then replace it with the other time.
        marks = ""
...
rest_button = Button(text="Rest", highlightbackground=YELLOW, command=rest_timer)
